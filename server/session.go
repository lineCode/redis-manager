...
//创建会话管理器(sessionName:在浏览器中session的名字;maxLifeTime:最长生命周期)
func NewSessionMgr(maxLifeTime int64) *SessionMgr {
...
func (mgr *SessionMgr) EndSession(w http.ResponseWriter, r *http.Request) {
	sessionID := mgr.CheckTokenValid(w, r)
	if sessionID == "" {
		return
	} else {
		mgr.mLock.Lock()
...
//判断Session的合法性（每进入一个页面都需要判断合法性）
func (mgr *SessionMgr) CheckTokenValid(w http.ResponseWriter, r *http.Request) string {
	var session = r.Header["X-Token"]
	var sessionID string
	if session == nil || len(session) == 0 {
		c, err := r.Cookie("Admin-Token")
		if err != nil {
			return ""
		}
		sessionID, _ = url.PathUnescape(c.Value)
	} else {
		sessionID = session[0]
	}
	mgr.mLock.Lock()
